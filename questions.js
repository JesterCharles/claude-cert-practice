...
window.QUESTIONS = [
  ...
  },
  {
    ...
    "explanations": {
      "C": "The hub-and-spoke pattern centralizes error handling, observability, retries, and information routing in the coordinator. That single seam makes the system easier to reason about: there is one place to add logging, one place to decide on retry policy, one place to fall back to an alternate subagent. Subagents stay focused on their domain task. You apply this whenever you orchestrate multiple specialized agents under one root. It beats the runner-up because splitting errors across \"transient vs permanent\" reintroduces the duplicated, scattered logic the centralization is meant to avoid.",
  ...
  },
  {
    ...
    "explanations": {
      "B": "The right framing balances ownership and stability. If you own all the MCP servers and the canonical format isn't going to drift, normalizing at the source is the cleanest pattern — every consumer benefits, and the agent stays simple. But when servers are third-party, heterogeneous, or evolving at different rates, PostToolUse hooks give you a single place in the agent to enforce uniformity without coordinating across teams or vendors. You apply this whenever you're choosing where in the stack to place a cross-cutting transformation. It beats the runner-up because hooks aren't *always* better; the answer is genuinely context-dependent.",
  ...
  },
  {
    ...
    "explanations": {
      "B": "Overly broad tool descriptions cause overuse because the model interprets them as catch-alls. Narrowing the description to a specific purpose and pointing the model to the right alternative for adjacent cases is the canonical fix. You apply this any time a single tool is absorbing calls that should be handled by simpler tools. It beats the removal option because `query_repo` still has a legitimate niche for semantic search; the fix is to restore boundaries, not delete the capability.",
  ...
  },
  {
    ...
    "explanations": {
      "D": "Replacing a generic tool with a constrained alternative is the canonical pattern: `load_document` advertises a narrower purpose, validates inputs structurally, and rejects out-of-scope URLs at the contract level. Both selection (the model sees a document-specific tool) and safety (input validation) improve. You apply this any time a generic tool is being misused and a narrower tool can serve the legitimate use cases. It beats the runtime-allowlist approach because the allowlist hides intent — the model still thinks `fetch_url` is general-purpose and will keep trying disallowed URLs.",
  ...
  },
  {
    ...
    "explanations": {
      "B": "`tool_choice: \"any\"` is exactly designed for the \"must call some tool, but not a specific one\" case. It guarantees the model produces a tool call rather than conversational text, while leaving discretion over which tool. You apply this whenever the downstream contract requires structured output and any tool in the set produces a valid result. It beats forced selection because forcing a specific tool removes the flexibility the scenario requires.",
  ...
  },
  {
    ...
    "explanations": {
      "C": "MCP resources are designed exactly for content catalogs that should be visible to the agent without consuming a tool call. Moving the topic catalog to a resource means the agent has it available continuously, removing the per-turn listing call. You apply this any time a catalog is queried for orientation rather than action. It beats turn-level caching because resources are the protocol-supported way to do this and integrate cleanly with how Claude reasons over available data.",
  ...
  },
  {
    ...
    "explanations": {
      "A": "Grep is the built-in for content search across files, exactly the right tool for finding callers — usages of a function name across many files. You apply Grep any time you need to find code that references a specific identifier, error message, or import. It beats Glob because file names do not tell you which files *use* `parseDate`; you need to inspect contents. The pattern \"find callers\" is the canonical Grep workflow.",
  ...
  },
  {
    ...
    "explanations": {
      "D": "Project-scoped slash commands live in `.claude/commands/` inside the repository so they're shared via version control and load for every clone. This matches the requirement precisely: ship with the project, not the user. You'd reach for project-scoped placement whenever the command encodes team workflows that should be uniformly available to anyone working on the codebase. It beats user-scoped placement because user-scoped commands exist only on the individual's machine and would not propagate to teammates or CI.",
  ...
  },
  {
    ...
    "explanations": {
      "C": "The design intent of path-scoped rules is to load only when relevant to reduce irrelevant context and token usage; tight globs match that intent and minimize cross-rule interference. This applies the principle that conditional loading is most effective when the condition is precise. You'd use tight globs whenever the rule's applicability is narrow (e.g., `**/*.test.tsx` for tests, `db/migrations/**` for migrations). It beats over-broad globs because broad patterns recreate the always-loaded-monolith problem the system was designed to solve.",
  ...
  },
  {
    ...
    "explanations": {
      "A": "The Explore subagent is purpose-built for isolating verbose discovery output and returning summaries to preserve main conversation context, which is exactly the scenario described. This applies the principle of subagent context isolation during exploration-heavy phases. You'd use Explore whenever discovery would otherwise crowd out budget for downstream reasoning and implementation. It beats running plan mode in the main session because plan mode and its discovery still consume the parent context — Explore moves the verbose work into a fork.",
  ...
  },
  {
    ...
    "explanations": {
      "B": "The interview pattern — having Claude ask questions to surface considerations the developer may not have anticipated — is designed exactly for unfamiliar domains where the developer suspects there are unknowns. This applies the principle that explicit elicitation before implementation prevents costly rework on overlooked requirements. You'd use the interview pattern any time you're entering a domain where you don't know what you don't know. It beats writing exhaustive test suites because tests presuppose you already know the edge cases — the interview pattern helps you discover them.",
  ...
  },
  {
    ...
    "explanations": {
      "D": "Session context isolation is the principle: a Claude session that already produced an implementation has reasoning anchored to that implementation and is less effective at critically reviewing it. An independent review instance, lacking those anchors, is more rigorous. This applies the CI principle of separating generation from review by session boundary. You'd use this same reasoning whenever a workflow has a generation step and a review step in the same pipeline — they should be different sessions. It beats reliance on randomness because the asymmetry isn't random — it's a predictable consequence of context.",
  ...
  },
  {
    ...
    "options": {
      "A": "Document team testing standards, valuable test criteria, and fixtures in CLAUDE.md so CI-invoked Claude Code uses them.",
  ...
  },
  {
    ...
    "explanations": {
      "B": "The principle is replacing open-ended evaluative criteria (\"could be improved\") with explicit inclusion/exclusion categories. The current prompt invites the model to flag anything, so it does. Defining what to report (bugs, security) versus skip (style, local patterns) directly constrains the scope. This same approach applies wherever an LLM produces too much output because the goal is under-specified — narrow the definition rather than add filters on top of noise. It beats the runner-up because a cap on findings still surfaces the wrong ten items, just hides the other 37.",
  ...
  },
  {
    ...
    "options": {
      ...
      "D": "Add 2–4 few-shot examples covering each citation style: inline-with-bib, author-year, and footnote-embedded, with correct outputs."
    },
    "correct": "D",
    "plausibility": {
      "D": "plausible-correct",
  ...
  },
  {
    ...
    "explanations": {
      "B": "The principle is that forcing a specific named tool via `tool_choice: {\"type\": \"tool\", \"name\": \"...\"}` ensures a particular extraction runs before downstream steps. This is the canonical pattern when a pipeline step has a known required tool and ordering matters. You'd use it whenever step ordering is non-negotiable and prompt-based ordering hints are insufficient. It beats the runner-up because exposing only one tool but using `\"auto\"` still lets the model respond with text instead of calling the tool, breaking the guarantee.",
  ...
  },
  {
    ...
    "explanations": {
      "A": "The principle is retry-with-error-feedback: appending the specific validation error to the retry prompt guides the model toward correction. By including the failed extraction and the precise sum mismatch (\"line items sum to 1,247 but total is 1,250\"), the model can self-correct on the second pass. This pattern is the canonical fix for semantic errors that are detectable but not preventable by the schema. It beats the runner-up because unprincipled retries sample from the same error distribution — you'll get many retries with the same mistake before a lucky pass.",
  ...
  },
  {
    ...
    "explanations": {
      "D": "The principle is adding a `detected_pattern` field to structured findings to enable systematic analysis of false positive patterns. When you can group dismissals by the underlying pattern that triggered the finding (\"unused-async-await\", \"unhandled-promise\"), you discover which categories are consistently disliked and can iterate prompts targeted at those patterns. This pattern applies whenever you need actionable signal from user reactions to bulk findings — categorize the cause, not just the effect. It beats the runner-up because confidence scores from instruction-tuned models are poorly calibrated and don't tell you which prompt section needs work.",
  ...
  },
  {
    ...
    "explanations": {
      "B": "The principle is designing self-correction validation flows: extract `calculated_total` alongside `stated_total` to flag discrepancies during extraction. The model performs the arithmetic explicitly as part of its output, making mismatches detectable without external math, and forces the model to reconcile its own line items with the stated total. This pattern applies broadly: extract derived values alongside source values to enable consistency checks. It beats the runner-up because a single `validation_passed` boolean is unreliable self-assessment, while the dual-field design produces ground-truth-checkable values.",
  ...
  },
  {
    ...
    "explanations": {
      "D": "The principle is calculating batch submission frequency from the SLA: if the batch API can take up to 24 hours and your SLA is 30 hours, the maximum wait time before submission is 30 − 24 = 6 hours; submitting every 4 hours gives a comfortable safety margin. This pattern applies to any batch-based SLA: subtract the worst-case batch latency from the SLA to get your maximum submission window. It beats the runner-up because once-daily submission can leave a document submitted just after the midnight cutoff waiting up to 48 hours total in the worst case.",
  ...
  },
  {
    ...
    "options": {
      ...
      "B": "Switch to a verification pass by an independent Claude instance that self-reports confidence on each finding for calibrated routing.",
  ...
  },
  {
    ...
    "explanations": {
      "D": "This question targets the skill of modifying upstream agents to return structured data when downstream agents have limited context budgets. Prose summaries are dense in tokens but sparse in machine-parseable signal, which is exactly the wrong tradeoff for a context-constrained synthesizer. Structured outputs (facts, citations, relevance scores) compress information losslessly for the downstream consumer and preserve provenance, which is why the synthesis agent stops conflating sources. Reach for this pattern any time you control both ends of an agent-to-agent handoff and the downstream side is the constrained one.",
  ...
  },
  {
    ...
    "explanations": {
      "D": "The question is a direct test of recognizing the lost-in-the-middle attention pattern. The symptom — reliable extraction from start and end, omissions from the middle — is the canonical fingerprint. You should diagnose this whenever document length is the new variable and the failure is positional rather than content-type-related. Knowing the name and mechanism unlocks the right mitigations: re-organize so critical content lives at the boundaries, place a key-findings prompt section at the start, or chunk the input. It beats the prompt-length runner-up because prompt brevity affects task framing, not positional attention.",
  ...
  },
  {
    ...
    "options": {
      ...
      "D": "Instruct the agent to ask the customer for an additional identifier (email, phone, or order number) before choosing."
    },
    "correct": "D",
    "plausibility": {
      "D": "plausible-correct",
  ...
  },
  {
    ...
    "options": {
      ...
      "B": "Have the subagent retry locally on transient failures and propagate only what it can't resolve, with attempts and partial results.",
  ...
  },
  {
    ...
    "options": {
      ...
      "D": "The coordinator is terminating the entire workflow on a single subagent failure, which The canonical guidance identifies as an anti-pattern."
    },
    "correct": "D",
    "plausibility": {
      "D": "plausible-correct",
    ...
    "explanations": {
      ...
      "B": "Prose vs. structured outputs is a separate the principle here concern and doesn't address the termination-on-failure anti-pattern."
    }
  },
  {
    "id": "d5-q20",
  ...
  },
  {
    ...
    "taskStatement": "5.3",
    "difficulty": "medium",
    "stem": "Your document-analysis subagent occasionally hits a parsing edge case where it gets 60% through a long PDF before encountering an unparseable embedded table. It currently returns `{\"status\":\"error\",\"reason\":\"parse_failure\"}` and the coordinator discards the 60% of successful analysis. What change best aligns with the principle here guidance?",
    "options": {
  ...
  },
  {
    ...
    "explanations": {
      ...
      "C": "Indefinite sessions accumulate exactly the degradation the principle here warns about.",
  ...
  },
  {
    ...
    "explanations": {
      "D": "The canonical guidance explicitly recommends summarizing key findings from one exploration phase before spawning subagents, then injecting summaries into initial context. The principle is that a curated, condensed handoff gives each subagent the signal it needs without the noise that would dilute its working memory. Apply this whenever you fan out from a coordinator that has accumulated context the subagents need but shouldn't inherit verbosely. It beats the transcript-passing runner-up because the transcript carries forward all the verbose exploration that was the original problem.",
  ...
  },
  {
    ...
    "explanations": {
      "A": "This applies the skill of using scratchpad files to record key findings and counteract context degradation when revisiting topics. The principle is that the specific entities discovered earlier (the `AuthHandler` class name) deserve durable storage outside the conversation, so revisiting the topic loads the precise reference rather than relying on Claude's degraded recall. Apply this any time exploration cycles back through previously visited topics. It beats the one-topic-per-session runner-up because it preserves the natural exploration flow rather than constraining the human.",
  ...
  },
  {
    ...
    "explanations": {
      "C": "The canonical pattern specifies stratified random sampling of high-confidence extractions for ongoing error-rate measurement and novel pattern detection. The principle is that stratification ensures every segment (document type, field) gets its own independent error estimate, so a problem in one stratum can't hide behind a healthy aggregate. Apply this anywhere you've reduced uniform human review and want to keep visibility into per-segment drift. It beats the flat-random runner-up because flat random under-samples small but important segments.",
  ...
  },
  {
    ...
    "taskStatement": "5.5",
    "difficulty": "medium",
    "stem": "You've decided to automate high-confidence extractions across all document types. Aggregate accuracy is 96%. A team member proposes a \"ship it\" plan that turns off human review immediately. What pre-deployment check most directly aligns with the principle here guidance before reducing review?",
    "options": {
  ...
  },
  {
    ...
    "options": {
      "A": "Require subagents to output structured claim-source mappings — URLs, document names, and excerpts — preserved through synthesis.",
  ...
  },
  {
    ...
    "options": {
      ...
      "D": "Have the document-analysis subagents output both values with full source attribution, letting the coordinator reconcile them."
    },
    "correct": "D",
    "plausibility": {
      "D": "plausible-correct",
  ...
  },
  {
    ...
    "options": {
      ...
      "C": "Require subagents to include publication date and data-collection date range in structured outputs for temporal accuracy.",
  ...
  },
  {
    ...
    "taskStatement": "5.6",
    "difficulty": "medium",
    "stem": "Your synthesis agent renders all findings as uniform prose paragraphs — financial figures, news quotes, and technical performance benchmarks alike. Readers complain that financial data is hard to compare across companies in paragraph form, and benchmark data feels muddled. You want to render content types in their appropriate forms. Which approach aligns with the principle here guidance?",
    "options": {
  ...
  },
  {
    ...
    "options": {
      ...
      "B": "Source attribution was lost during synthesis when findings were compressed into prose without claim-source mappings.",
    ...
    "explanations": {
      "B": "This is a direct test of the principle here's headline concept: source attribution is lost during summarization steps when findings are compressed without preserving claim-source mappings. The principle is that prose synthesis is inherently lossy unless explicit structure carries source attribution through every compression step. Recognize this pattern any time a final report has claims that can't be traced back to specific inputs. The fix is structured claim-source mappings that propagate through synthesis. It beats the access-to-subagent runner-up because the underlying issue is the lost mapping, not whether the raw outputs were shareable.",
  ...
  },
  {
    ...
    "options": {
      ...
      "D": "Preserve original source characterizations and methodological context in the synthesis output so consumers can compare claims."
    },
    "correct": "D",
    "plausibility": {
      "D": "plausible-correct",
